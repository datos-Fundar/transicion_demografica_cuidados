...
Uso las bases de Estructura por sexo y edad de la población por provincia del Censo 2022 de INDEC - Cuadro 4.15. Provincia del Neuquén. Total de población, por sexo registrado al nacer e índice de feminidad, según edad y departamento. Año 2022 [https://www.indec.gob.ar/indec/web/Nivel4-Tema-2-41-165] 
...
#me faltan los totales de mujeres y varones
...
tipologias <- demanda_depto %>% 
  mutate(diferencia_Infancias = prop_grupo_depto40_Infancias - prop_grupo_depto22_Infancias,
         ...
         tipo_dif_PI = case_when(diferencia_Infancias < -0.02 ~ "A",
                                 diferencia_Infancias >= -0.02 & diferencia_Infancias < -0.01 ~ "I",
                                 diferencia_Infancias < 0 & diferencia_Infancias >= -0.01 ~ "B",
         ...
         cambio = case_when(prop_grupo_depto22_Infancias < 0.062 & prop_grupo_depto22_PAM < 0.1624 & prop_grupo_depto40_PAM > 0.2064 ~ "ni_PI",
                            prop_grupo_depto22_Infancias > 0.062 & prop_grupo_depto40_PAM > 0.2064 ~ "PI_PM",
                            prop_grupo_depto22_PAM > 0.1624 & prop_grupo_depto40_Infancias > 0.0516 ~ "PM_PI",
                            prop_grupo_depto22_Infancias > 0.062 & prop_grupo_depto40_Infancias > 0.0516 ~ "PI_PI",
                            prop_grupo_depto22_PAM > 0.1624 & prop_grupo_depto40_PAM > 0.2064 ~ "PM_PM",
                            TRUE ~ "nini"))

write.xlsx(tipologias, "tipologias.xlsx")

resumen_cambio <- tipologias %>% 
  group_by(name_prov, cambio) %>% 
  summarise(total = n()) %>% 
  pivot_wider(names_from = cambio, values_from = total) %>% 
  mutate(total = PI_PI + PI_)

resumen_combinacion_dif <- tipologias %>% 
  group_by(name_prov, cambio) %>% 
  summarise(total = n()) %>% 
  pivot_wider(names_from = cambio, values_from = total) %>% 
  mutate(total = PI_PI + PI_)
...
# Alto PI: < -0.02
# Intermedio PI: < -0.01 & > -0.02
# Bajo PI: > -0.01
# Cero PI: = 0

# Alto PI: > 0.06
# Intermedio PI: < 0.06 & > 0.04
# Bajo PI: < 0.04 & > 0.02
# Cero PM: = 0
```
